 ...
 func (q Script) ForRange(i string, from Number, to Number, block func(Number, Script)) {
  ...
  func (q Script) ForEach(i, v string, list Type, block func(Number, Type, Script)) {
	id, value, statement := q.lang.ForEach(i, v, convert(list))
	
	q.indent()
	q.write(statement)
	q.depth++
		block(q.wrap(id).(Number), q.wrap(value), q)
	q.depth--
	q.indent()
